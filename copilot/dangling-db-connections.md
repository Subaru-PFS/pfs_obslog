...
これらの処理は元々CPU負荷の高い処理（画像変換、FITSファイル読み込み）でしたが、forkベースの並列処理を廃止したことで、fork時の接続プール継承問題が根本的に解消されました。

**注意**: `async def`エンドポイント内で同期的なブロッキング処理を行うと、イベントループがブロックされ、他の非同期リクエストの処理が待たされます。これはパフォーマンス上の課題として残っていますが、DB接続のidle問題とは別の話です。必要に応じて`run_in_executor`などで改善できます。
