...
INHERITED_COLUMN_FIXES = {
    # 親クラス側
    "AgcData": ["flags"],       # 親: AgcMatch が継承
    "CobraTarget": ["flags"],   # 親: CobraMatch が継承
    # 子クラス側
    "AgcMatch": ["flags"],
    "CobraMatch": ["flags"],    # 親: CobraMove が継承
    "CobraMove": ["flags"],
}
...
def _apply_postprocess_fixes(output_path: Path) -> None:
    ...
    - Add warning filter for SAWarning about inherited columns
    """
    content = output_path.read_text()
    original_content = content

    # Add file header with warning filter
    content = _add_file_header(content)
...
# Header to prepend to generated models file
FILE_HEADER = '''# ruff: noqa: E501
"""SQLAlchemy models generated from database schema.

This file is auto-generated by backend/devel/generate_models.py.
...
# Suppress SQLAlchemy warnings about inherited columns
# These warnings occur in joined table inheritance when both parent and child
# have columns with the same name, even with use_existing_column=True
warnings.filterwarnings(
    "ignore",
    message=r"Implicitly combining column .* under attribute 'flags'",
    category=SAWarning,
)
...
def _add_file_header(content: str) -> str:
    """Add file header with docstring and warning filter."""
    # Check if header already exists
    if "auto-generated by backend/devel/generate_models.py" in content:
        return content
