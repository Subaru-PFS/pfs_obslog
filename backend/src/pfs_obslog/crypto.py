...
def default_key_generator():
    base = bytes.fromhex(Path(os.environ['SECRET_KEY_BASE_FILE']).read_text())
    if len(base) < 32:  # pragma: no cover
        raise ValueError(
            f'secret key base is too short: {base}. '
            f'It must be longer the 32 bytes')
    return KeyGenerator(base)


class MessageEncryptor:
    def __init__(self, salt: str, *, key_generator: KeyGenerator | None = None):
        self._secret = (key_generator or default_key_generator())(salt, key_size=32)

    def encrypt_and_sign(self, payload: str, *, expires_in: float | None = None) -> str:
